...
class Notch10a:
    ...
    def mae_parameter_search_on_training(self, x_data_training, y_data_training, x_data_test, y_data_test, base_k,
        ...
        parameters = [{
            'C': np.logspace(base=self.step, start=-47, stop=27, num=(27 + 47 + 1), endpoint=True),
            'gamma': np.logspace(base=self.step, start=-39, stop=28, num=(28 + 39 + 1), endpoint=True),
            'tol': np.logspace(base=self.step, start=-38, stop=20, num=(20 + 38 + 1), endpoint=True),
            'epsilon': np.logspace(base=self.step, start=-34, stop=16, num=(16 + 34 + 1), endpoint=True)
        }]
        print(parameters)
